...
const SPREADSHEET_ID = "1_pN_s9UnCdrFzjaBiEcz6XH75-LVqqrmMOsRvySenXY"; // ID del spreadsheet para Acción Social
const SPREADSHEET_ID_TWO = "1TESmWwkzNwOJ3RQ0XMx9VbegR7ecx1cIA5DcO7cWbYs"; // ID del spreadsheet para Tránsito
const SPREADSHEET_ID_THREE = "14sZKKAdy9r01jrC7nTH9gstrTWyd-6gUUeTVzNXGtpY"; // ID del spreadsheet para Castración
...
app.get("/api/get-turnos-accion", async (req, res) => {
  try {
    ...
    rows.forEach((row) => {
      // Verificar si la fila tiene un día
      if (row[0] && row[0].trim() !== "") {
        diaActual = row[0].trim();
        turnos[diaActual] = [];
      }

      // Si ya se tiene un día actual, se agregan los turnos
      if (diaActual) {
        turnos[diaActual].push({
          ...
          disponible: row[5] || "No disponible", // Asegúrate de que esto se ajuste a tus necesidades
        });
      }
    });
    ...
    Object.keys(turnos).forEach((dia) => {
      ...
      for (let i = existingTurnos; i < 10; i++) {
        turnos[dia].push({
          ...
          fecha: "",
          nombre: "",
          disponible: "No disponible", // Asegúrate de que los turnos vacíos tengan un valor para disponibilidad
        });
      }
    });

    res.json(turnos); // Enviar los datos al frontend como JSON
  } catch (error) {
    console.error("Error al leer datos de Google Sheets:", error);
...
app.get("/api/bot-status", (req, res) => {
  // Lógica para determinar el estado del bot
  res.json({ status: false }); // Ejemplo
});
